...
```bash
daml build
daml codegen js .daml/dist/MyBorrowApp-0.1.0.dar -o ui/daml.js
cd ui
npm install
npm run-script build
zip -r ../MyBorrowApp-ui.zip build
```

Next you need to create a ledger on [Daml Hub], upload the files
`.daml/dist/MyBorrowApp-0.1.0.dar` (created by the `daml build` command)
and `MyBorrowApp-ui.zip` (created by the `zip` command based on the result
